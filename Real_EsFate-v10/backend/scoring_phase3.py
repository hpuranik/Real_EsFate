...
def compute_tract_score_phase3(tract_id: str) -> Dict[str, Any]:
    ...
    1. Tier 1: Research-validated signals (+ real ACS income data where available)
    2. Tier 2: Government data — static mock profiles PLUS live HMDA lending data
       (HMDA: Rosen 1974, JPE 82(1); Hwang & Lin 2016, Housing Policy Debate 26(2))
    ...
    # Gather all signals
    tier1_signals = get_tier1_signals(tract_id)
    tier2_data = get_all_tier2_signals(tract_id)
    tier3_schema = get_tier3_schema()

    # HMDA lending data: real API call (ffiec.cfpb.gov), mock fallback if unreachable.
    # lending_activity_score is 0-100 normalized origination volume -- high = capital
    # inflow, a leading appreciation signal (Rosen 1974; PCRG 2019).
    hmda_data = get_tract_lending(tract_id)
    tier2_data["hmda_lending"] = {
        "value": hmda_data["lending_activity_score"],
        "source": hmda_data["data_source"],
        "confidence": 0.88 if hmda_data["data_source"] != "mock_fallback" else 0.60,
        "supplemental": {
            "loan_originations": hmda_data["loan_originations"],
            "denial_rate": hmda_data["denial_rate"],
            "investor_share": hmda_data["investor_share"],
        },
    }
...
def _get_tenant_resources(tract_id: str) -> list:
    """Return relevant tenant resources and rights for Allegheny County / Pittsburgh.

    Verified June 2026. These are the actual organizations serving this county --
    a prior version of this function returned Philadelphia (215-area-code, phila.gov)
    resources to Pittsburgh residents, which is exactly the kind of confident-looking
    wrong answer this project's own principles are supposed to prevent.
    """
    return [
        "Neighborhood Legal Services Association (NLSA) - free legal aid for eviction, "
        "security deposits, habitability: 412-434-6002 or 800-482-3076 (toll-free)",
        "RentHelpPGH - eviction prevention outreach & rental assistance navigation "
        "for Allegheny County: renthelppghresources.org",
        "Just Mediation Pittsburgh - free landlord-tenant mediation before an eviction filing",
        "Pittsburgh Union of Regional Renters - tenant organizing and union support",
        "Dial 211 - Allegheny County's general health & human services referral line",
    ]
